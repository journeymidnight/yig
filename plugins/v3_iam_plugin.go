...
func (a IamV3Client) GetKeysByUid(uid string) (credentials []common.Credential, err error) {
	...
	slog.Info(10, "url is:", url)
	signUrl := SignWithRequestURL("GET", url, a.SercetAccessKey)
	slog.Info(20, "Url of GetKeysByUid send request to IAM :", signUrl)
	...
	if err != nil {
		slog.Error(5, "GetKeysByUid send request failed", err)
	...
	slog.Info(20, "GetKeysByUid to IAM return status ", response.Status)
	if response.StatusCode != 200 {
		slog.Warn(5, "GetKeysByUid to IAM failed return code = ", response.StatusCode)
...
func (a IamV3Client) GetCredential(accessKey string) (credential common.Credential, err error) {
	...
	if len(akParams) == 2 && akParams[0] == helper.CONFIG.LogDelivererAK {
		...
		signUrl := SignWithRequestURL("GET", url, a.SercetAccessKey)
		slog.Info(20, "Url of GetCredential send request to IAM :", signUrl)

		request, _ := http.NewRequest("GET", signUrl, nil)
		response, err := a.httpClient.Do(request)
		if err != nil {
			slog.Error(5, "GetCredential send request failed", err)
		...
		if err != nil {
			slog.Error(20, "Read IAM response err:", err)
			return credential, err
		}
		defer body.Close()
		s := string(jsonBytes)
		...
		if response.StatusCode != 200 {
			slog.Warn(5, "GetCredential to IAM failed return code = ", response.StatusCode)
