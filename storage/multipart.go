...
func (yig *YigStorage) CopyObjectPart(targetObject *meta.Object, data io.Reader, credential common.Credential,
	sseRequest datatype.SseRequest) (result datatype.PutObjectResult, err error) {

	multipart, err := yig.MetaStorage.GetMultipart(targetObject.BucketName, targetObject.Name, targetObject.UploadId)
	if err != nil {
		return
	}
	...
	now := time.Now().UTC()
	part := meta.Part{
		PartNumber:           targetObject.PartId,
	...
	// remove possible old object in Ceph
	if part, ok := multipart.Parts[targetObject.PartId]; ok {
		RecycleQueue <- objectToRecycle{
			location: multipart.Metadata.Location,
			pool:     multipart.Metadata.Pool,
			objectId: part.ObjectId,
...
func (yig *YigStorage) CompleteMultipartUpload(credential common.Credential, targetObject *meta.Object,
	...
	helper.Logger.Println(20, "Upload parts:", uploadedParts, "uploadId:", targetObject.UploadId)
	for i := 0; i < len(uploadedParts); i++ {
		if uploadedParts[i].PartNumber != i+1 {
			helper.Logger.Println(20, "uploadedParts[i].PartNumber != i+1; i:", i, "uploadId:", targetObject.UploadId)
			err = ErrInvalidPart
			return
		}
		part, ok := multipart.Parts[i+1]
		if !ok {
			helper.Logger.Println(20, "multipart.Parts[i+1] does not exist; i:", i, "uploadId:", targetObject.UploadId)
			err = ErrInvalidPart
			return
		}
		...
		if part.Etag != uploadedParts[i].ETag {
			helper.Logger.Println(20, "part.Etag != uploadedParts[i].ETag;",
				"i:", i, "Etag:", part.Etag, "reqEtag:", uploadedParts[i].ETag, "uploadId:", targetObject.UploadId)
			err = ErrInvalidPart
			return
		}
		var etagBytes []byte
		...
		if err != nil {
			helper.Logger.Println(20, "hex.DecodeString(part.Etag) err;", "uploadId:", targetObject.UploadId)
			err = ErrInvalidPart
			return
		}
