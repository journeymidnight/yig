...
func (cluster *CephCluster) Put(poolname string, data io.Reader) (oid string,
	...
	var c AioCompletion
	pending := list.New()
	var current_upload_window = helper.Conf.UploadMinChunkSize /* initial window size as MIN_CHUNK_SIZE, max size is MAX_CHUNK_SIZE */
	...
	for {
		...
		if elapsed_time.Nanoseconds() > 2*int64(expected_time) {
			if slow_count > 2 && current_upload_window > helper.Conf.UploadMinChunkSize {
			...
			current_upload_window = current_upload_window << 1
			if current_upload_window > helper.Conf.UploadMaxChunkSize {
				current_upload_window = helper.Conf.UploadMaxChunkSize
			}
			slow_count = 0
		}
...
func (cluster *CephCluster) Append(poolname string, existName string, data io.Reader,
	...
	if poolname != backend.BIG_FILE_POOLNAME && poolname != backend.GLACIER_FILE_POOLNAME {
		return oid, 0,
