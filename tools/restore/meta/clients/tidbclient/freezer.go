...
func (t *TidbClient) ListFreezersNeedContinue(maxKeys int, status common.Status) (retFreezers []Freezer, err error) {
	var count int
	var marker string
	marker = ""
	for {
...
func (t *TidbClient) UploadFreezerStatus(bucketName, objectName, version string, status, statusSetting common.Status) (err error) {
...
func (t *TidbClient) UploadFreezerBackendInfo(targetFreezer *Freezer) (err error) {
	sqltext := "update restoreobjects set pool=?,size=? where bucketname=? and objectname=? and version=?;"
	_, err = t.Client.Exec(sqltext, targetFreezer.Pool, targetFreezer.Size, targetFreezer.BucketName, targetFreezer.Name, targetFreezer.VersionId)
	if err != nil {
		return err
	}
	return nil
...
func (t *TidbClient) PutFreezer(freezer *Freezer, status common.Status, tx Tx) (err error) {
	if tx == nil {
...
func (t *TidbClient) DeleteFreezer(bucketName, objectName, version string, objectType ObjectType, createTime uint64, tx Tx) (err error) {
	if tx == nil {
		tx, err = t.Client.Begin()
		if err != nil {
			return err
	...
	_, err = tx.(*sql.Tx).Exec(sqltext, bucketName, objectName, version)
	if err != nil {
		return err
	}
