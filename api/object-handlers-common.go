...
func checkObjectPreconditions(w http.ResponseWriter, r *http.Request, object *meta.Object, brand Brand) error {
	...
	ifModifiedSinceHeader := r.Header.Get(brand.GetHeaderFieldKey(XCopySourceIfModifiedSince))
	...
	ifUnmodifiedSinceHeader := r.Header.Get(brand.GetHeaderFieldKey(XCopySourceIfUnmodifiedSince))
	...
	ifMatchETagHeader := r.Header.Get(brand.GetHeaderFieldKey(XCopySourceIfMatch))
	...
	// one specified
	ifNoneMatchETagHeader := r.Header.Get(brand.GetHeaderFieldKey(XCopySourceIfNoneMatch))
