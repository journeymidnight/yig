...
func toStringLikeFuncString(n name, key Key, values utils.StringSet) string {
...
func (f stringLikeFunc) evaluate(values map[string][]string) bool {
	for _, v := range values[f.k.Name()] {
		if !f.values.FuncMatch(utils.Match, v).IsEmpty() {
			return true
		}
	}
...
func validateStringLikeValues(n name, key Key, values utils.StringSet) error {
