...
func toStringEqualsFuncString(n name, key Key, values utils.StringSet) string {
...
func (f stringEqualsFunc) evaluate(values map[string][]string) bool {
	...
	return !f.values.Intersection(utils.CreateStringSet(requestValue...)).IsEmpty()
}
...
func validateStringEqualsValues(n name, key Key, values utils.StringSet) error {
...
func NewStringEqualsFunc(key Key, values ...string) (Function, error) {
	sset := utils.CreateStringSet(values...)
