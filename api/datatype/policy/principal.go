...
func (p Principal) Intersection(principal Principal) utils.StringSet {
...
func (p Principal) Match(principal string) bool {
	for _, pattern := range p.AWS.ToSlice() {
		if utils.MatchSimple(pattern, principal) {
			return true
		}
	}
...
func NewPrincipal(principals ...string) Principal {
	return Principal{AWS: utils.CreateStringSet(principals...)}
}
