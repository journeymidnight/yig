...
func PureInitialize(cephConfigPattern string,
	...
	cephConfigFiles, err := filepath.Glob(cephConfigPattern)
	if err != nil || len(cephConfigFiles) == 0 {
		return nil, NewError(InCephFatalError, "no ceph conf found", nil)
	}
...
func (cluster *CephCluster) doSmallAppend(poolname string, oid string, offset uint64, length int64, data io.Reader) (size uint64, err error) {
	...
	if err != nil {
		return 0, NewError(InCephFatalError, "doSmallAppend bad ioctx of pool "+poolname, err)
	}
...
func (cluster *CephCluster) Put(poolname string, data io.Reader) (oid string,
	...
	pool, err := cluster.Conn.OpenPool(poolname)
	if err != nil {
		return oid, 0, NewError(InCephFatalError, "Put bad pool name "+poolname, err)
	}
	defer pool.Destroy()

	striper, err := pool.CreateStriper()
	if err != nil {
		return oid, 0, NewError(InCephFatalError, "Put bad ioctx of pool "+poolname, err)
	}
	defer striper.Destroy()
	...
	for {
		...
		if err != nil && err != io.EOF {
			...
			return oid, 0, NewError(InCephFatalError, fmt.Sprintf("Put read from client failed. pool:%s oid:%s", poolname, oid), err)
		}
		if count == 0 {
			break
		}
		...
		if err != nil {
			c.Release()
			drain_pending(pending)
			return oid, 0, NewError(InCephFatalError, fmt.Sprintf("Put bad io. pool:%s oid:%s", poolname, oid), err)
		}
		pending.PushBack(c)
	...
	if slice_offset > 0 {
		...
		if err != nil {
			c.Release()
			return oid, 0, NewError(InCephFatalError, fmt.Sprintf("Put error writing remaining data, pool:%s oid:%s", poolname, oid), err)
		}
		pending.PushBack(c)
	}
...
func (cluster *CephCluster) Append(poolname string, existName string, data io.Reader,
	...
	pool, err := cluster.Conn.OpenPool(poolname)
	if err != nil {
		return oid, 0, NewError(InCephFatalError, fmt.Sprintf("Append bad poolname %s", poolname), err)
	}
	defer pool.Destroy()

	striper, err := pool.CreateStriper()
	if err != nil {
		return oid, 0, NewError(InCephFatalError, fmt.Sprintf("Append bad ioctx of pool %s", poolname), err)
	}
	defer striper.Destroy()
	...
	for {
		...
		if err != nil {
			return oid, 0, NewError(InCephFatalError, fmt.Sprintf("Append bad io. pool:%s oid:%s", poolname, oid), err)
		}
