...
func TestBucketVersioning(t *testing.T) {
	...
	status, err := sc.GetBucketVersion(TestBucket)
	assert.Equal(t, err, nil, "GetBucketVersion err")
	assert.Equal(t, status, "", "GetBucketVersion not empty")

	err = sc.PutBucketVersion(TestBucket, s3.BucketVersioningStatusEnabled)
	...
	err = sc.PutBucketVersion(TestBucket, s3.BucketVersioningStatusSuspended)
...
func TestObjectVersionEnabled(t *testing.T) {
	...
	defer func() {
		...
		sc.DeleteObjectVersion(TestBucket, TestKey, "null")
		out, _ := sc.ListObjectVersions(TestBucket, "", "", "", 1000)
		for _, v := range out.Versions {
			sc.DeleteObjectVersion(TestBucket, TestKey, *v.VersionId)
		}
	...
	out, err := sc.GetObjectOutPut(TestBucket, TestKey)
	...
	putOut, err := sc.PutObjectOutput(TestBucket, TestKey, TestValue)
	...
	delOut, err := sc.DeleteObjectOutput(TestBucket, TestKey)
	...
	_, err = sc.GetObject(TestBucket, TestKey)
	assert.NotEqual(t, err, nil, "object should return not found err")

	out, err = sc.GetObjectVersionOutPut(TestBucket, TestKey, markerVersion)
	...
	delOut, err = sc.DeleteObjectVersion(TestBucket, TestKey, markerVersion)
	...
	delOut, err = sc.DeleteObjectVersion(TestBucket, TestKey, objectVersion)
	...
	delOut, err = sc.DeleteObjectVersion(TestBucket, TestKey, "null")
...
func TestListObjectVersions(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		putObjOut, err := sc.PutObjectOutput(TestBucket, TestKey, TestValue)
		assert.Equal(t, err, nil, "PutObject err")
		assert.NotEqual(t, putObjOut.VersionId, nil, "PutObject err")
		t.Log("VersionId", i, ":", *putObjOut.VersionId)
	}

	listOut, err := sc.ListObjectVersions(TestBucket, "", "", "", 100)
