...
func (s3client *S3Client) ChangeObjectStorageClass(bucketName, key string, storageClass types.StorageClass) (err error) {
	params := &s3.CopyObjectInput{
		Bucket:            aws.String(bucketName),
		Key:               aws.String(key),
		CopySource:        aws.String("/" + bucketName + "/" + key),
		MetadataDirective: aws.String("REPLACE"),
		StorageClass:      aws.String(storageClass.ToString()),
	}
