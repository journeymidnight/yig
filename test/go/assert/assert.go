...
// NotEqual asserts that got and want are not equal as defined by
// reflect.DeepEqual. The test fails with msg if they are equal.
func NotEqual(t *testing.T, got, want interface{}, msg ...string) {
	if x := notEqual(2, got, want, msg...); x != "" {
		t.Fatal(x)
	}
}

func notEqual(skip int, got, want interface{}, msg ...string) string {
	if reflect.DeepEqual(got, want) {
		return fail(skip, "not want %v %s", got, want, strings.Join(msg, " "))
	}
	return ""
}
